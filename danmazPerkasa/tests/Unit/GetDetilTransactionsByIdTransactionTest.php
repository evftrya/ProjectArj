...
namespace Tests\Unit\Controllers {
    ...
    class getDetilTransactionsByIdTransactionTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_101_getDetilTransactionsByIdTransaction_returns_joined_rows()
        {
            $this->requiresTablesOrSkip(['detail__transactions', 'products', 'transaksis'], 'tables required');

            $this->seedUser(['id_User' => 2]);
            $pid = $this->seedProduct(['id_product' => 5]);

            $this->seedTransaksi(['id' => 37, 'id_user' => 2]);

            $statusCol = $this->pickColCI('detail__transactions', ['status', 'Status']);

            $row = [
                'id_Detail_transaction' => 701,
                'id_User' => 2,
                'id_product' => $pid,
                'qty' => 1,
                'Total' => 10000,
            ...
            $rows = (new DetailTransactionController())->getDetilTransactionsByIdTransaction(37);
