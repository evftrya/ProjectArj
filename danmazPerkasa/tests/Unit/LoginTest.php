...
namespace Tests\Feature {
     ...
     * ========================= */
        ...
        public function test_UTB_003_login_email_found_redirect_login()
        {
            $this->seedUser(['emailUser' => 'a@a.com']);
            $req = $this->makeRequest(['emailUser' => 'a@a.com', 'passwordUser' => 'x'], 'POST');

            $res = (new AccountController())->login($req);

            $this->assertTrue(method_exists($res, 'getTargetUrl'));
            $this->assertStringContainsString('/Login', $res->getTargetUrl());
            $this->assertSame(2, session('user_id'));
            $this->assertSame('A B', session('user_name'));
        }

        public function test_UTB_004_login_email_not_found_redirect_with_pesan()
        {
            $req = $this->makeRequest(['emailUser' => 'x@x.com', 'passwordUser' => 'x'], 'POST');
            $res = (new AccountController())->login($req);
