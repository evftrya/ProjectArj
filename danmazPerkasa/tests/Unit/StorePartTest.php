...
namespace Tests\Unit\Controllers {
    ...
    class storePartTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_095_storePart_creates_detail_transaction_checkout()
        {
            $this->requiresTablesOrSkip(['users', 'products', 'detail__transactions'], 'users/products/detail__transactions tables required');

            $this->seedUser(['id_User' => 2]);
            $pid = $this->seedProduct(['id_product' => 5, 'price' => 10000, 'stok' => 10, 'type' => 'Product']);
            $this->setSession(['user_id' => 2]);

            (new DetailTransactionController())->storePart($pid);

            $this->assertTrue(
                DB::table('detail__transactions')
                    ->where('id_User', 2)
