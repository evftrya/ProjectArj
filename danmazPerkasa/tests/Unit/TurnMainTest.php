...
namespace Tests\Unit\Controllers {
    ...
    class turnMainTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_107_turnMain_sets_only_one_main_photo()
        {
            $this->requiresTablesOrSkip(['products', 'photos'], 'products/photos tables required');

            $pid = $this->seedProduct(['id_product' => 5]);

            $this->insertCaseSafe('photos', [
                'id_Photo' => 1,
                'PhotosName' => 'a.jpg',
                'id_product' => $pid,
                'isMain' => 1,
            ...
            (new PhotosController())->turnMain(2, $pid);

            $main1 = DB::table('photos')->where('id_Photo', 1)->first();
            $main2 = DB::table('photos')->where('id_Photo', 2)->first();

            $this->assertSame(0, (int)$main1->isMain);
            $this->assertSame(1, (int)$main2->isMain);
        }
    }
}
