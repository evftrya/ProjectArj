...
namespace Tests\Unit\Controllers {
    ...
    class ListPartTest extends BaseControllerTestCase
    {
     ...
     * =========================================================== */
        ...
        public function test_TestCase_143_listPart_returns_view_with_joined_parts()
        {
            $this->mockAllViews();

            $this->requiresTablesOrSkip(['products', 'photos', 'category_parts', 'ref_category_parts'], 'tables required');
            ...
            $this->seedProduct(['id_product' => 5001, 'type' => 'Part']);
            $this->seedPhoto(['id_Photo' => 5001, 'id_product' => 5001]);
            DB::table('products')->where('id_product', 5001)->update(['mainPhoto' => 5001]);

            $this->seedCategoryParts(5001, 1);

            $resp = (new ProductsController())->ListPart();
            $this->assertStringContainsString('view:User.Pelanggan.listPart', (string)$resp->getContent());
        }
    }
}