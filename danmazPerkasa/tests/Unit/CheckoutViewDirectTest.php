...
namespace Tests\Unit\Controllers {
    ...
    class CheckoutViewDirectTest extends BaseControllerTestCase
    {
       public function test_UTB_045_checkoutViewDirect_not_logged_redirect_login()
        {
            $this->setSession(['user_id' => 0, 'direction' => null]);
            $res = (new DetailTransactionController())->CheckoutViewDirect(5);
            $this->assertStringContainsString('/login', strtolower($res->getTargetUrl()));
        }
     ...
     * =========================================================== */
        ...
        public function test_TestCase_104_CheckoutViewDirect_creates_temp_checkout_and_returns_view_stub()
        {
            $this->requiresTablesOrSkip(['users', 'products', 'detail__transactions', 'addresses', 'provinces', 'cities', 'notifications'], 'tables required');

            $this->mockViewHelper();

            $this->seedUser(['id_User' => 2]);
            $this->seedProvinceCity();
            $this->seedAddress(['id_user' => 2, 'ShippingRate' => json_encode([['costs' => []]])]);
            $pid = $this->seedProduct(['id_product' => 5, 'price' => 10000, 'stok' => 10]);

            $this->setSession(['user_id' => 2, 'direction' => null]);

            $resp = (new DetailTransactionController())->CheckoutViewDirect($pid);

            $this->assertSame('view-stub', $resp);
            $this->assertTrue(DB::table('detail__transactions')->where('id_User', 2)->where('id_product', $pid)->exists());
        }
     ...
     * =========================================================== */
        ...
        public function test_TestCase_119_checkoutViewDirect_redirects_when_direction_not_null()
        {
            $this->seedUser();
            $this->setSession(['user_id' => 2, 'Role' => 'User', 'direction' => '/somewhere']);

            $resp = (new DetailTransactionController())->CheckoutViewDirect(5);
            $this->assertInstanceOf(RedirectResponse::class, $resp);
            $this->assertStringContainsString('/somewhere', $resp->getTargetUrl());
        }
    }
}