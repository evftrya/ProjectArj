...
namespace Tests\Unit\Controllers {
    ...
    class DeleteAccountTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_085_DeleteAccount_deletes_user_and_redirects_manage_user()
        {
            $this->requiresTablesOrSkip(['users'], 'users table required');

            $uid = $this->seedUser(['id_User' => 9, 'emailUser' => 'del@a.com']);
            $this->assertTrue(DB::table('users')->where('id_User', 9)->exists());

            $resp = (new AccountController())->DeleteAccount(9);

            $this->assertInstanceOf(RedirectResponse::class, $resp);
            $this->assertFalse(DB::table('users')->where('id_User', 9)->exists());
        }
    }
}