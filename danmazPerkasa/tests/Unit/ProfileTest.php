...
namespace Tests\Unit\Controllers {
    ...
    class ProfileTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_089_Profile_redirects_to_login_when_not_authenticated()
        {
            $this->setSession(['user_id' => 0, 'direction' => null]);

            $resp = (new BaseController())->Profile('Info', new AccountController());

            $this->assertInstanceOf(RedirectResponse::class, $resp);
            $this->assertSame('/Profile/Info', session('direction'));
        }

        public function test_TestCase_090_Profile_returns_view_stub_when_authenticated_and_wht_info()
        {
            $this->requiresTablesOrSkip(['users', 'notifications'], 'users/notifications tables required');

            $this->mockViewHelper();

            $uid = $this->seedUser(['id_User' => 2, 'namaUser' => 'A B', 'passwordUser' => '123']);
            $this->setSession(['user_id' => $uid, 'direction' => null]);
