...
namespace Tests\Unit\Controllers {
    ...
    class getDataWithoutPasswordTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_082_getDataWithoutPassword_excludes_password_field_and_returns_flags()
        {
            $this->requiresTablesOrSkip(['users'], 'users table required');

            $uid = $this->seedUser(['id_User' => 2, 'isActive' => 'active', 'isDelete' => 'no']);
            $rows = (new AccountController())->getDataWithoutPassword($uid);

            $this->assertNotEmpty($rows);
            $row = $rows[0];
            $this->assertSame($uid, $row->id);
            $this->assertObjectNotHasProperty('pw', $row);
            $this->assertObjectHasProperty('isActive', $row);
            $this->assertObjectHasProperty('isDelete', $row);
        }
    }
}