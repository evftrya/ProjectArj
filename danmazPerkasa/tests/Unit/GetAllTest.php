...
namespace Tests\Unit\Controllers {
    ...
    class getAllTest extends BaseControllerTestCase
    {
     ...
     * =========================================================== */
        ...
        public function test_TestCase_148_getAll_returns_only_done_payments()
        {
            $this->requiresTablesOrSkip(['transaksis', 'users'], 'tables required');

            $this->seedUser();
            $this->seedTransaksi(['id' => 7001, 'id_user' => 2, 'Status_Pembayaran' => 'Done']);
            $this->seedTransaksi(['id' => 7002, 'id_user' => 2, 'Status_Pembayaran' => 'Waiting']);

            $rows = (new TransaksiController())->getAll();
            $this->assertNotEmpty($rows);
            $this->assertSame(7001, (int)$rows[0]->id);
        }
    }
}