...
namespace Tests\Unit\Controllers {
    ...
    class deleteTempCheckoutTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_097_deleteTempCheckout_deletes_temp_checkout_items_for_user()
        {
            $this->requiresTablesOrSkip(['users', 'products', 'detail__transactions'], 'tables required');
            ...
            if ($statusCol) $row[$statusCol] = 'TempCheckout';

            $this->insertCaseSafe('detail__transactions', $row);

            (new DetailTransactionController())->deleteTempCheckout();

            $this->assertFalse(DB::table('detail__transactions')->where('id_Detail_transaction', 99)->exists());
        }
     ...
     * =========================================================== */
        ...
        public function test_TestCase_120_deleteTempCheckout_returns_outofpages_when_not_logged_in()
        {
            $this->mockAllViews();
            $this->setSession(['user_id' => null]);

            $resp = (new DetailTransactionController())->deleteTempCheckout();
            $this->assertStringContainsString('view:OutOfPages', (string)$resp->getContent());
        }
    }
}