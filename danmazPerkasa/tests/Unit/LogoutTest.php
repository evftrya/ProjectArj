...
namespace Tests\Feature {
     ...
     * ========================= */
        ...
        public function test_UTB_011_logout_with_status_redirect_with_pesan()
        {
            $this->setSession(['user_id' => 2, 'Role' => 'User']);
            $req = $this->makeRequest(['status' => 1], 'POST');

            $res = (new AccountController())->Logout($req);
            $this->assertStringContainsString('/Login', $res->getTargetUrl());
            $this->assertSame(0, session('user_id'));
        }

        public function test_UTB_012_logout_without_status_redirect_login()
        {
            $this->setSession(['user_id' => 2, 'Role' => 'User']);
