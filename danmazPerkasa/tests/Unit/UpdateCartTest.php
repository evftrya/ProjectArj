...
namespace Tests\Unit\Controllers {
    ...
    class UpdateCartTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_096_UpdateCart_updates_qty_and_total()
        {
            ...
            $dtId = $this->seedDetailTransaction([
            ...
            $req = Request::create('/cart/update', 'POST', ['qty' => 3]);
            $resp = (new DetailTransactionController())->UpdateCart($req, $pid, $dtId);

            $this->assertInstanceOf(JsonResponse::class, $resp);
            $row = DB::table('detail__transactions')->where('id_Detail_transaction', $dtId)->first();
            $this->assertSame(3, (int)$row->qty);
            $this->assertSame(30000, (int)($row->Total ?? $row->total ?? 0));
        }
    }
}