...
namespace Tests\Feature {
    ...
    class cekExistEmailTest extends TestCase
    {
        use RefreshDatabase;
     ...
     * ========================= */
        ...
        public function test_UTB_007_cekExistEmail_data_found_wht_login_wrong_pw()
        {
            $this->seedUser(['emailUser' => 'a@a.com', 'passwordUser' => '123']);
            $res = (new AccountController())->cekExistEmail('a@a.com', '999', 'Login');
            $this->assertSame('Wrong Password', $this->decodeJsonResponse($res)['message']);
        }

        public function test_UTB_008_cekExistEmail_data_found_wht_login_good()
        {
            $this->seedUser(['emailUser' => 'a@a.com', 'passwordUser' => '123']);
            $res = (new AccountController())->cekExistEmail('a@a.com', '123', 'Login');
            $this->assertSame('Good', $this->decodeJsonResponse($res)['message']);
        }

        public function test_UTB_009_cekExistEmail_data_found_wht_register_email_exist()
        {
            $this->seedUser(['emailUser' => 'a@a.com']);
            $res = (new AccountController())->cekExistEmail('a@a.com', null, 'Register');
            $this->assertStringContainsString('Email have been Exist', $this->decodeJsonResponse($res)['message']);
        }

        public function test_UTB_010_cekExistEmail_data_not_found_email_not_registered()
        {
            $res = (new AccountController())->cekExistEmail('x@x.com', '123', 'Login');
            $this->assertSame('Email Not Registered', $this->decodeJsonResponse($res)['message']);
        }
    }
}