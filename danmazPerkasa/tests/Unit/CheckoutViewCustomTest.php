...
namespace Tests\Unit\Controllers {
    ...
    class CheckoutViewCustomTest extends BaseControllerTestCase
    {
        /* ===========================================================
     ...
     * =========================================================== */
        ...
        public function test_TestCase_098_CheckoutViewCustom_returns_view_stub_and_builds_custom_data()
        {
            $this->requiresTablesOrSkip(['users', 'products', 'photos', 'addresses', 'provinces', 'cities', 'notifications'], 'tables required');
            ...
            $this->seedAddress([
                'id_user' => 2,
                'ShippingRate' => json_encode([['costs' => []]]),
            ]);

            $p1 = $this->seedProduct(['id_product' => 5, 'type' => 'Product', 'mainPhoto' => 1, 'weight' => 1000]);
            $this->seedPhoto(['id_Photo' => 1, 'id_product' => $p1, 'isMain' => 1]);

            $p2 = $this->seedProduct(['id_product' => 6, 'type' => 'Product', 'mainPhoto' => 2, 'weight' => 1000, 'nama_product' => 'P2']);
            $this->seedPhoto(['id_Photo' => 2, 'id_product' => $p2, 'isMain' => 1]);

            $this->setSession(['user_id' => 2]);

            $req = Request::create('/checkout/custom', 'POST', [
                'dataPart' => $p1 . '-' . $p2,
            ]);

            $resp = (new DetailTransactionController())->CheckoutViewCustom($req);
     ...
     * =========================================================== */
        ...
        public function test_TestCase_121_checkoutViewCustom_single_item_branch_throws_due_to_controller_bug_but_is_covered()
        {
            $this->mockAllViews();
            ...
            $req = $this->makeRequest(['dataPart' => '5'], 'POST');

            $this->expectException(\ErrorException::class);
            (new DetailTransactionController())->CheckoutViewCustom($req);
        }
    }
}