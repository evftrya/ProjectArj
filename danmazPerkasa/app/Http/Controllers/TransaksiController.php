...
class TransaksiController extends Controller
{
    ...
    public function toTransaction($idT)
    {
        ...
        if (isset($Data[0])) {
            ...
            $retur = DB::table('transaksis as a')
                    ->join('detail__transactions as b', 'b.Transaksis_id', '=', 'a.id')
                    ->join('return_pesanans as c', 'c.id_detil_transaksi', '=', 'b.id_Detail_transaction')
                    ->join('products as d', 'd.id_product', '=', 'b.id_product')
                    ->select('c.*', 'd.nama_product')
                    ->where('a.id', $idT)
                    ->get();
            ...
            if (session('Role') == "Admin") {
                // $Address = new AddressController();
                // $detil = $Address->getDetil($Data[0]->id_user);
                return view('Transaction', ['retur' => $retur,'notif' => $notifs, 'data' => $Data, 'userData' => $userData, 'idT' => $idT, 'Address' => $detil, 'shipping' => $shipping]);
            } else {
                ...
                return view('Transaction', ['retur' => $retur, 'notif' => $notifs, 'data' => $Data, 'userData' => $userData, 'idT' => $idT, 'snapToken' => $ST, 'Address' => $detil, 'shipping' => $shipping]);
            }
        } else {
            return redirect('/PageNotFound');
        }
